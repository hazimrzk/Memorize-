...
struct EmojiMemoryGameView: View {
    ...
    var shuffle: some View{
        Button("\(Image(systemName: "shuffle")) Shuffle"){
            withAnimation(.easeInOut(duration: 2.5)){
                viewModel.shuffle()
            }
            
        }.font(.headline.bold()).foregroundColor(viewModel.theme.colorCards)
            .padding(.horizontal, 35.0).padding(.vertical, 15.0)
    ...
    var NewGameButton: some View {
        Button {
            ...
            VStack() {
                Text("\(Image(systemName: "plus.diamond")) New")
                    .font(.headline.bold()).foregroundColor(viewModel.theme.colorCards)
                        .padding(.horizontal, 45.0).padding(.vertical, 15.0)
                        .background(Color.white)
                        .cornerRadius(10)
                        .shadow(radius: 5)
            }
